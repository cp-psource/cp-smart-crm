...
//		$annotazioni.=" (<a href='".$url.$link."'>".$url.$link."</a>)";
		...
		$_annotazioni=$timestamp_date."~".$user_firstname." ".$user_lastname. "~".__($n,'cpsmartcrm').": <b>".strtoupper($oggetto)."</b> ".$annotazioni."|";
		$result=$wpdb->query( 
			$wpdb->prepare(
        ...
        if ($file)
        {
            ...
            foreach ($all_rows as $riga)
            {
                ...
                else
                {
                    ...
                    $row_err="";
                    if (!$riga["first_name"])
                        $row_err.=sprintf( __( 'Missing field: %s', 'cpsmartcrm' ), "first_name")."; ";
                    if (!$riga["last_name"])
                        $row_err.=sprintf( __( 'Missing field: %s', 'cpsmartcrm' ), "last_name")."; ";
                    if (!$riga["company"])
...
<fieldset>
    ...
    <tr>
	    ...
	    <td colspan="2" style="padding-top:20px"><textarea rows="3" cols="50" name="esito" id="esito">'.stripslashes($riga->esito).'</textarea></td>
          ...
          <span class="button button-primary _flat" id="save_activity_from_modal" data-id="'.$ID.'">'.__( 'Save', 'cpsmartcrm' ).'</span>
          <span class="button button-secondary _flat _reset" id="configreset">'.__( 'Reset', 'cpsmartcrm' ).'</span>
...
<div class="col-md-3">
	<label style="font-weight:normal;margin-top:10px;padding:8px 0">
		<?php _e("Print gros amount","cpsmartcrm")?>
	</label>
...
<div class="col-md-6">
	<label style="font-weight:normal;margin-top:10px;padding:8px 0">
		<?php _e("Print 'PAID' if paid","cpsmartcrm")?>
	</label>
